    ...
    try {
        ...
        if (
          ...
          <div class="flex items-center mb-3">
            ...
            <div class="flex-1">
              <h5 class="font-medium text-slate-800">${section.Name
                    }</h5>
              <p class="text-xs text-slate-500">${section.Type} - ${section.TemplateType
                    }</p>
            </div>
          </div>
          ${section.Description
    ...
    if (canonicalInput) {
        canonicalInput.addEventListener("input", function () {
            ...
            if (preview) {
                preview.textContent =
                    this.value ||
                    `https://example.com/${document.querySelector('input[name="Code"]').value
                    }`;
            }
        });
    }
