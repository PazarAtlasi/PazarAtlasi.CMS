...
const SectionModal = (function () {
    ...
    function collectItemDataRecursive(
        ...
        const itemType =
            ...
            parseInt(itemCard.dataset.nestedType) || 0;
    ...
    async function addSectionItem() {
        ...
        while (retryCount < maxRetries) {
            ...
            console.log(
                `Waiting for template configuration... retry ${retryCount + 1
                }`
            );
            ...
            nestedCard.innerHTML = `
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs font-medium text-slate-600">
                        <i class="fas fa-link mr-1"></i> ${itemType} #${currentCount + 1
                }
                    </span>
              ...
              ${templates
                ...
                <button type="button"
                  ...
                  <div class="flex items-center justify-between text-xs text-slate-400 mt-2 pt-2 border-t border-slate-100">
                    <span><i class="fas fa-tag mr-1"></i>${template.templateType
                        }</span>
            ...
            const modalHTML = `
                <div id="itemTypeSelectionModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    ...
                    .map(
                        (item, index) => `
                                    <button type="button"
                                            onclick="SectionModal.addSectionItemByType(${templateId}, '${item.itemType
                            }', ${sectionId}); SectionModal.closeItemTypeSelectionModal();"
                                            class="p-4 border-2 border-slate-200 hover:border-blue-500 hover:bg-blue-50 rounded-lg transition-all text-left group">
                                        <div class="flex items-center mb-2">
                                            <i class="fas fa-${item.uiConfiguration
                                ?.iconClass || "cube"
    ...
    function debugFieldCollection() {
        ...
        allCards.forEach((card) => {
            ...
            directFieldContainers.forEach((container) => {
                ...
                const isTranslatable =
                    container.dataset.translatable === "true";
                const fieldType = getFieldTypeFromContainer(container);
                const dataFieldType = container.dataset.fieldType;

                console.log(
                    `    ✓ Direct field: ${fieldKey}, level: ${fieldLevel}, translatable: ${isTranslatable}, type: ${fieldType}, data-type: ${dataFieldType}`
                );
            });
