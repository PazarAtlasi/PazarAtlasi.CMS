...
function showSectionItemModal(html) {
    ...
    const existingModal = document.getElementById('sectionItemModalOverlay');
    if (existingModal) {
    ...
    const modalContent = modalOverlay.querySelector('.section-item-modal');
    if (modalContent) {
...
function initializeSectionItemModal() {
    ...
    const activeMediaBtn = document.querySelector('.media-type-btn.active');
    if (activeMediaBtn) {
...
function handleItemTypeChange(itemType) {
    ...
    const mediaSettingsPanel = document.getElementById('mediaSettingsPanel');
    ...
    if (mediaSettingsPanel) {
        ...
        console.log('Media Panel Display:', mediaSettingsPanel.style.display); // Debug
    } else {
...
function updateTranslationLabels(typeValue) {
    const titleLabels = document.querySelectorAll('.translation-item label');
    
    // Customize labels based on type
    if (typeValue === 1) { // Text
        // Show text-focused fields
    } else if (typeValue === 2 || typeValue === 9 || typeValue === 10) { // Image, Picture, Video
...
function selectMediaType(mediaType) {
    ...
    const selectedBtn = document.querySelector(`[data-media-type="${mediaType}"]`);
    if (selectedBtn) {
    ...
    switch (mediaType) {
        case 'Image':
            document.getElementById('singleImageUpload').style.display = 'block';
            break;
        case 'ImageSlider':
            document.getElementById('sliderImagesUpload').style.display = 'block';
            break;
        case 'Video':
...
async function handleSingleImageUpload(input) {
    ...
    try {
        ...
        if (result.success) {
            ...
            const placeholder = document.getElementById('singleImagePlaceholder');
            if (placeholder) {
...
async function handleSliderImageUpload(input, slotIndex) {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Update preview
            const slot = document.querySelector(`.slider-image-slot[data-slot="${slotIndex}"]`);
            if (slot) {
                slot.innerHTML = `
                    <div class="relative">
                        <img src="${result.url}" alt="Slider ${slotIndex + 1}" class="w-full h-24 object-cover rounded" />
            ...
            showNotification(`Slider image ${slotIndex + 1} uploaded successfully`, 'success');
        } else {
            showNotification(result.message || 'Failed to upload image', 'error');
        }
    } catch (error) {
    ...
    if (slot) {
        slot.innerHTML = `
            <div class="h-24 flex flex-col items-center justify-center">
                ...
                <p class="text-xs text-slate-500">Image ${slotIndex + 1}</p>
            </div>
            <input type="file" id="sliderImage${slotIndex}" accept="image/*" class="hidden" onchange="handleSliderImageUpload(this, ${slotIndex})" />
            <button type="button" onclick="document.getElementById('sliderImage${slotIndex}').click()" 
                    class="mt-2 py-1 px-2 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded text-xs">
...
async function handleVideoUpload(input) {
    ...
    try {
        ...
        if (result.success) {
            ...
            showNotification(result.message || 'Failed to upload video', 'error');
        }
    } catch (error) {
        console.error('Upload error:', error);
        showNotification('An error occurred while uploading', 'error');
...
async function saveSectionItem() {
    try {
        // Gather form data
        const itemId = parseInt(document.getElementById('itemId').value) || 0;
        ...
        if (mediaTypes.includes(type)) {
            switch (currentMediaType) {
                case 'Image':
                    ...
                    break;
                case 'ImageSlider':
                    mediaAttributes.sliderImages = sliderImages.filter(img => img !== '');
        ...
        document.querySelectorAll('.translation-item').forEach(item => {
            ...
            const title = item.querySelector('.translation-title')?.value || '';
            const description = item.querySelector('.translation-description')?.value || '';
            const buttonText = item.querySelector('.translation-button-text')?.value || '';
            const buttonUrl = item.querySelector('.translation-button-url')?.value || '';
        ...
        if (result.success) {
            ...
            showNotification(result.message || 'Failed to save section item', 'error');
            if (result.error) {
...
function showUploadProgress(message) {
    ...
    document.getElementById('uploadProgressMessage').textContent = message;
    progressDiv.style.display = 'flex';
}

function hideUploadProgress() {
    const progressDiv = document.getElementById('uploadProgressOverlay');
    if (progressDiv) {
